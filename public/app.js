...
const renderStands = () => {
  ...
  state.stands.forEach((s) => {
    ...
    card.innerHTML = `
      <div style="display:flex;justify-content:space-between;align-items:center;">
        ...
        <div class="form-row">
          ...
          ${token ? `<button class="danger" data-del="${s.id}" title="Удалить стенд">Удалить</button>` : ''}
        </div>
      </div>
      <div class="tags">
    ...
    `;
    card.querySelector('button[data-id]').onclick = () => selectStand(s.id);
    if (token) {
      const delBtn = card.querySelector(`button[data-del="${s.id}"]`);
      if (delBtn) delBtn.onclick = async () => {
        if (!confirm('Удалить стенд со всеми связанными записями?')) return;
        try {
          await api(`/stands/${s.id}`, { method: 'DELETE' });
          if (state.selectedStand && state.selectedStand.id === s.id) {
            state.selectedStand = null;
            el('standHeader').innerHTML = '<div class="muted">Выберите стенд слева, чтобы увидеть детали</div>';
            el('tabs').innerHTML = '';
...
const selectStand = async (id) => {
  ...
  el('standHeader').innerHTML = `
    <div style="display:flex;justify-content:space-between;align-items:flex-start;gap:12px;">
      ...
      ${token ? `<button class="danger" id="standDeleteBtn" title="Удалить стенд и связанные данные">Удалить стенд</button>` : ''}
    </div>
  `;
  if (token) {
    const del = document.getElementById('standDeleteBtn');
    if (del) del.onclick = async () => {
      if (!confirm('Удалить стенд и все связанные записи?')) return;
      try {
        await api(`/stands/${stand.id}`, { method: 'DELETE' });
        state.selectedStand = null;
        await loadStands();
        el('standHeader').innerHTML = '<div class="muted">Выберите стенд слева, чтобы увидеть детали</div>';
        el('tabs').innerHTML = '';
...
const renderDocumentsTab = (container) => {
  ...
  if (token) {
    ...
    linkForm.className = 'form-row';
    linkForm.innerHTML = `
      <input type="text" id="docUrl" placeholder="Ссылка на файл (http...)" style="min-width:240px;" />
      <input type="text" id="docUrlTitle" placeholder="Название (опционально)" />
      <label class="muted"><input type="checkbox" id="docUrlInline" /> Inline</label>
      <button id="uploadDocUrlBtn">Загрузить по ссылке</button>
    `;
    linkForm.querySelector('#uploadDocUrlBtn').onclick = async () => {
      const url = el('docUrl').value.trim();
      if (!url) { alert('Нужна ссылка'); return; }
      try {
        await api(`/stands/${state.selectedStand.id}/documents`, {
          method: 'POST',
          body: {
            url,
            title: el('docUrlTitle').value || undefined,
            editable_inline: el('docUrlInline').checked,
          },
        });
        await loadDocuments(state.selectedStand.id);
        renderTabs('documents');
    ...
    if (token) {
      ...
      upload.innerHTML = `
        <div class="form-row">
          ...
          <button data-up="${d.id}">Загрузить файл</button>
        </div>
        <div class="form-row">
          <input type="text" id="verUrl-${d.id}" placeholder="Ссылка на версию (http...)" style="min-width:260px;" />
          <input type="text" id="verUrlDesc-${d.id}" placeholder="Комментарий" />
          <label class="muted"><input type="checkbox" id="verUrlActive-${d.id}" /> Актуальная</label>
          <button data-upurl="${d.id}">Загрузить по ссылке</button>
        </div>
      `;
      ...
      upload.querySelector(`[data-upurl="${d.id}"]`).onclick = async () => {
        const url = upload.querySelector(`#verUrl-${d.id}`).value.trim();
        if (!url) { alert('Нужна ссылка'); return; }
        try {
          await api(`/distributions/${d.id}/versions`, {
            method: 'POST',
            body: {
              url,
              description: upload.querySelector(`#verUrlDesc-${d.id}`).value,
              is_active: upload.querySelector(`#verUrlActive-${d.id}`).checked,
            },
          });
          await loadDistributions(state.selectedStand.id);
          renderTabs('distributions');
